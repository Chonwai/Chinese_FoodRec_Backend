...
class DishServices implements BaseServicesInterface
{
    ...
    {
        $data = DAOSimpleFactory::createDishDAO()->getAll();
    ...
    {
        $data = DAOSimpleFactory::createDishDAO()->getSpecify($request);

        return Utils::responseMessage($data, 'unknownProblems');
    }

    public function responseSpecifyByFilter($request)
    {
        $data = DAOSimpleFactory::createDishDAO()->getSpecifyByFilter($request);
